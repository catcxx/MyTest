...
/**
 * 
 * @author kerryfish 关于java中链表的操作 1. 求单链表中结点的个数: getListLength 2. 将单链表反转:
 *         reverseList（遍历），reverseListRec（递归） 3. 查找单链表中的倒数第K个结点（k > 0）:
 *         reGetKthNode 4. 查找单链表的中间结点: getMiddleNode 5. 从尾到头打印单链表:
 *         reversePrintListStack，reversePrintListRec（递归） 6. 已知两个单链表pHead1
 *         和pHead2 各自有序，把它们合并成一个链表依然有序: mergeSortedList, mergeSortedListRec 7.
 *         对单链表进行排序,listSort（归并）,insertionSortList（插入） 8. 判断一个单链表中是否有环: hasCycle
 *         9. 判断两个单链表是否相交: isIntersect 10. 已知一个单链表中存在环，求进入环中的第一个节点:
 *         getFirstNodeInCycle, getFirstNodeInCycleHashMap 11.
 *         给出一单链表头指针head和一节点指针delete，O(1)时间复杂度删除节点delete: deleteNode
 */
