 ...
 * mirrorCopyRec mirror, mirrorCopy 10.1 判断两个树是否互相镜像：isMirrorRec isMirror 11.
 * 求二叉树中两个节点的最低公共祖先节点： LAC 求解最小公共祖先, 使用list来存储path. LCABstRec 递归求解BST树. LCARec
 * 递归算法 . 12. 求二叉树中节点的最大距离：getMaxDistanceRec 13.
 * 由前序遍历序列和中序遍历序列重建二叉树：rebuildBinaryTreeRec 14.
 * 判断二叉树是不是完全二叉树：isCompleteBinaryTree, isCompleteBinaryTreeRec 15.
...
public class TreeDemo {
	...
	public static int getDepth(TreeNode root) {
		...
		while (!q.isEmpty()) {
			...
			if (curr == dummy) {
				depth++;
				if (!q.isEmpty()) { // 使用DummyNode来区分不同的层，
									// 如果下一层不是为空，则应该在尾部加DummyNode.
					q.offer(dummy);
				}
			}
	...
	/*
	 * 3. 前序遍历，中序遍历，后序遍历: preorderTraversalRec, preorderTraversal,
	 ...
	 * 1 / \ 2 3 / \ \ 4 5 6
	 * 
	 * if put into the stack directly, then it should be: 1, 2, 4, 5, 3, 6 in
	 * the stack. when pop, it should be: 6, 3, 5, 4, 2, 1
	 * 
	 * if I
	 */
	 ...
	 * http://blog.csdn.net/tang_jin2015/article/details/8545457 从左到右的后序
	 * 与从右到左的前序的逆序是一样的，所以就简单喽！ 哈哈 用另外一个栈进行翻转即可喽
	 */
	...
	/*
	 * 8. 判断两棵二叉树是否相同的树。 迭代解法 我们直接用中序遍历来比较就好啦
	 */
	...
	/*
	 * 
	 * 9. 判断二叉树是不是平衡二叉树：isAVLRec 1. 左子树，右子树的高度差不能超过1 2. 左子树，右子树都是平衡二叉树。
	 * 
	 */
	 ...
	 * 1 1 / \ 2 -----> 2 \ / 3 3
	 * 
	 * 应该可以使用任何一种Traversal 方法。 我们现在可以试看看使用最简单的前序遍历。
	 */
	 ...
	 * 1 1 / \ 2 -----> 2 \ / 3 3
	 * 
	 * 应该可以使用任何一种Traversal 方法。 我们现在可以试看看使用最简单的前序遍历。 前序遍历我们可以立刻把新建好的左右节点创建出来，比较方便
	 */
	...
	/*
	 * 11. 求二叉树中两个节点的最低公共祖先节点： Recursion Version: LACRec 1. If found in the left
	 * tree, return the Ancestor. 2. If found in the right tree, return the
	 * Ancestor. 3. If Didn't find any of the node, return null. 4. If found
	 * both in the left and the right tree, return the root.
	 */
	public static TreeNode LACRec(TreeNode root, TreeNode node1, TreeNode node2) {
		...
		// if no node is in the node, just recursively find it in LEFT and RIGHT
		// tree.
		TreeNode left = LACRec(root.left, node1, node2);
		TreeNode right = LACRec(root.right, node1, node2);

		if (left == null) { // If didn't found in the left tree, then just
							// return it from right.
			return right;
		} else if (right == null) { // Or if didn't found in the right tree,
									// then just return it from the left side.
			return left;
		}

		// if both right and right found a node, just return the root as the
		// Common Ancestor.
		return root;
	}
	 ...
	 * 
	 * 1. If found in the left tree, return the Ancestor. 2. If found in the
	 * right tree, return the Ancestor. 3. If Didn't find any of the node,
	 * return null. 4. If found both in the left and the right tree, return the
	 * root.
	 */
	public static TreeNode LCABstRec(TreeNode root, TreeNode node1, TreeNode node2) {
		...
		// if the values are smaller than the root value, just search them in
		// the left tree.
		if (root.val > max) {
			return LCABstRec(root.left, node1, node2);
		} else if (root.val < min) {
			// if the values are larger than the root value, just search them in
			// the right tree.
	...
	public static TreeNode LCA(TreeNode root, TreeNode r1, TreeNode r2) {
		...
		// If never find any node which is different, means Node 1 and Node 2
		// are the same one.
	 ...
	 * 
	 * 首先我们来定义这个距离： 距离定义为：两个节点间边的数目. 如： 1 / \ 2 3 \ 4 这里最大距离定义为2，4的距离，为3.
	 * 求二叉树中节点的最大距离 即二叉树中相距最远的两个节点之间的距离。 (distance / diameter) 递归解法： 返回值设计： 返回1.
	 * 深度， 2. 当前树的最长距离 (1) 计算左子树的深度，右子树深度，左子树独立的链条长度，右子树独立的链条长度 (2) 最大长度为三者之最：
	 * a. 通过根节点的链，为左右深度+2 b. 左子树独立链 c. 右子树独立链。
	 * 
	 * (3)递归初始条件： 当root == null, depth = -1.maxDistance = -1;
	 * 
	 */
	...
	/*
	 * 13. 由前序遍历序列和中序遍历序列重建二叉树：rebuildBinaryTreeRec We assume that there is no
	 * duplicate in the trees. For example: 1 / \ 2 3 /\ \ 4 5 6 /\ 7 8
	 * 
	 * PreOrder should be: 1 2 4 5 3 6 7 8 根 左子树 右子树 InOrder should be: 4 2 5 1
	 * 3 7 6 8 左子树 根 右子树
	 */
	...
	/*
	 * 14. 判断二叉树是不是完全二叉树：isCompleteBinaryTree, isCompleteBinaryTreeRec 进行level
	 * traversal, 一旦遇到一个节点的左节点为空，后面的节点的子节点都必须为空。而且不应该有下一行，其实就是队列中所有的
	 * 元素都不应该再有子元素。
	 */
	 ...
	 * 
	 * 
	 * 我们可以分解为： CompleteBinary Tree 的条件是： 1. 左右子树均为Perfect binary tree,
	 * 并且两者Height相同 2. 左子树为CompleteBinaryTree, 右子树为Perfect binary
	 * tree，并且两者Height差1 3. 左子树为Perfect Binary Tree,右子树为CompleteBinaryTree,
	 * 并且Height 相同
	 * 
	 * Base 条件： (1) root = null: 为perfect & complete BinaryTree, Height -1;
	 * 
	 * 而 Perfect Binary Tree的条件： 左右子树均为Perfect Binary Tree,并且Height 相同。
	 */
	...
	public static ReturnBinaryTree isCompleteBinaryTreeRecHelp(TreeNode root) {
		...
		/*
		 * CompleteBinary Tree 的条件是： 1. 左右子树均为Perfect binary tree,
		 * 并且两者Height相同(其实就是本树是perfect tree) 2. 左子树为CompleteBinaryTree,
		 * 右子树为Perfect binary tree，并且两者Height差1 3. 左子树为Perfect Binary
		 * Tree,右子树为CompleteBinaryTree, 并且Height 相同
		 */
	...
	/*
	 * 15. findLongest 第一种解法： 返回左边最长，右边最长，及左子树最长，右子树最长。
	 */
