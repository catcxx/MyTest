...
public class TestFile {
	/*
	 * createNewFile() delete() mkDir():创建一个文件目录。只有在上层文件目录存在的情况下，才能返回true
	 * mkDirs():创建一个文件目录。若上层文件目录不存在，一并创建 list() 字符串读 listFiles() 文件读
	 */
	@Test
	...
	/*
	 * exists() canWrite() canRead() isFile() isDirectory() lastModified()
	 * length()
	 * 
	 */
	@Test
	...
	/*
	 * 路径： 绝对路径：包括盘符在内的完整的文件路径 相对路径：在当前文件目录下的文件的路径
	 * 
